...
<body>
    <form id="generate-schema-form" class="schema-form">
        ...
        <textarea id="jsonTextarea" name="JSON" class="textarea" rows="7"><?php
  echo json_encode([
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateSchemaPreview() {
                ...
                // Add the foreign keys to the schema if they exist
                $('input[name="foreignKey[]"]').each(function() {
...
$('#fields-table').on('change', 'select[name="dataType[]"], select[name="indexType[]"]', function() {
    ...
    // Disable Index for Timestamp, Enum, Float, Double, Decimal, and Boolean and file
    if (["timestamp", "enum", "float", "double", "decimal", "boolean","file"].includes(dataType)) {
    ...
    // Adjust placeholders for specific data types
    if (dataType === "integer") {
        row.find('input[name="length[]"]').attr('placeholder', 'e.g., 123');
    } else if (dataType === "string") {
        row.find('input[name="length[]"]').attr('placeholder', 'e.g., 255');
    } else if (dataType === "enum") {
        row.find('input[name="length[]"]').attr('placeholder', 'e.g., value1,value2,value3'); // Enum values as comma-separated
    }else if (dataType === "file"){
        row.find('input[name="length[]"]').attr('placeholder', 'e.g., jpg, png, gif, pdf, *'); // Enum values as comma-separated
    }
    else if (dataType === "float" || dataType === "double" || dataType === "decimal") {
        row.find('input[name="length[]"]').attr('placeholder', 'e.g., 10,2'); // Precision and scale for float, double, decimal
    }

    updateSchemaPreview(); // Update schema preview after change
});
